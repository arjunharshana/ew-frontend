...
function Metric({ value, label, accent }) {
  return (
    <div className="flex flex-col gap-0.5 min-w-0 flex-1 overflow-hidden">
      <span 
        className={`mono text-2xl font-bold whitespace-nowrap overflow-hidden text-ellipsis ${!accent ? 'text-slate-900' : ''}`}
        style={accent ? { color: accent, textShadow: `0 1px 6px ${accent}40` } : { textShadow: '0 1px 4px rgba(0,0,0,0.05)' }}
      >
        {value}
      </span>
      <span className="text-[11px] md:text-[12px] font-medium text-slate-600 leading-[1.3] inline self-start max-w-full">
        {label}
      </span>
    </div>
  );
...
export default function PerformancePanel({ metrics }) {
  return (
    <div className="py-1">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 2xl:grid-cols-4 gap-x-4 gap-y-3 lg:gap-y-3 items-start">
        <Metric
